...
public static class HotelPrivateCameraExtra
{
    ...
    public static async Task<(bool Enabled, decimal Price)> GetAsync(AppDbContext db, int groomerId)
    {
        ...
        return hit == null
            ? (false, DefaultPrice)
            : (true, hit.Price < 0 ? DefaultPrice : hit.Price);
    }

    public static async Task SyncAsync(AppDbContext db, int groomerId, bool enabled, decimal? price = null)
    {
        ...
        var p = price is null or < 0 ? DefaultPrice : price.Value;
